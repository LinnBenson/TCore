...
use Bootstrap;
     ...
     */
    class BuildView {
        public $id, $request, $view, $share, $child, $plugPath, $code, $config, $resource, $cache;
        ...
        public function __construct( $data ) {
            ...
            if ( is_object( $this->share['view'] ) ) {
                ...
                $this->share = $this->superior->share;
                unset( $this->share['view'] );
            }else {
                $this->addSystemShare();
            }
            // 加载原始视图文件
            if ( !empty( $this->config['library'][$data['view']] ) ) {
                $codeFile = $this->plugPath.$this->config['library'][$data['view']];
            }else {
        ...
        public function render( $viewFileOriginalCode, $share = [] ) {
            try {
                ...
                Bootstrap::log( "TCore_View Error: {$this->view}", $e );
                return null;
            }
        }
