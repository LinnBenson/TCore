...
renderer.codespan = ({ text }) => {
    return `<code class="codeBlock copy" data-clipboard-text="${tc.markdown.protect( text )}">${tc.markdown.protect( text )}</code>`;
};
renderer.code = ({ text }) => {
    return `
        <div class="showCode codeBlock">
            <div class="showCodeHeader">
                <div></div>
                <div></div>
                <div></div>
                <i class="block bi-copy copy" data-clipboard-text="${tc.markdown.protect( text )}"></i>
            </div>
            <div class="showCodeContent">
                <pre class="code">${tc.markdown.protect( text )}</pre>
            </div>
        </div>
    `;
};
renderer.image = function({ href, text }) {
    return `
        <div class="showImg">
            <img src="${href}" alt="${tc.markdown.protect( text )}" />
            ${!empty( text ) ? `<p class="more">${text}</p>` : ''}
        </div>
    `;
};
marked.setOptions({ renderer });

tc.markdown.protect = ( text, revert = false ) => {
    // 默认字符转义
    text = revert ?
...
tc.markdown.render = ( text ) => {
    ...
    // 输出 HTML
    return `<div markdown>${marked.parse( text )}</div>`;
};